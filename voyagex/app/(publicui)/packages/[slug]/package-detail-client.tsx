...
const PackageDetailPage = () => {
  ...
  useEffect(() => {
    if (packageData?.id) {
      trackEvent("PACKAGE_VIEWED", "package", packageData.id);
    }
  }, [packageData?.id]);
  ...
  const handleBookNow = () => {
    ...
    trackEvent("BOOKING_STARTED", "package", packageData.id);
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {images.map((img: string, i: number) => (
                        <button key={i} onClick={() => setCurrentImageIndex(i)} className={`relative h-20 rounded-lg overflow-hidden ${i === currentImageIndex ? "ring-2 ring-green-600" : ""}`}>
                          <img src={getImageUrl(img)} alt="Thumbnail" loading="lazy" decoding="async" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "/agency-placeholder.jpg"; }} />
                        </button>
                      ))}
                    </div>
                  </>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                    {packageData.agencies?.logo ? <img src={getImageUrl(packageData.agencies.logo)} alt="" loading="lazy" decoding="async" className="w-12 h-12 object-cover" /> : <span className="text-gray-500 font-semibold">{(packageData.agencies?.name?.[0] || packageData.guides?.users?.firstName?.[0] || "V")}</span>}
                  </div>
                  ...
                  <button onClick={async () => {
                    ...
                    try {
                      ...
                      if (result.success && result.data?.id) {
                        trackEvent("MESSAGE_STARTED", "package", packageData.id);
