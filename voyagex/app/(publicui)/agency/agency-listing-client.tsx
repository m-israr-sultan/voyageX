  ...
  const AgencyCard = ({ agency }: { agency: Agency }) => (
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02] flex flex-col p-3 cursor-pointer h-full" onClick={() => handleCardClick(agency)}>
      <div className="w-full h-[180px] rounded-lg overflow-hidden relative mb-3">
        <img src={agency.logo} alt={agency.name} className="w-full h-full object-cover" loading="lazy" decoding="async" onError={(e) => { (e.target as HTMLImageElement).src = "/agency-placeholder.jpg"; }} />
      ...
      <div className="flex gap-2 mt-auto">
        ...
        <button onClick={(e) => handleViewDetails(agency, e)} className={`${agency.totalPackages > 0 ? "flex-1" : "w-full"} h-[32px] bg-[#E6F4EA] text-gray-900 text-sm font-medium rounded-lg hover:bg-[#D6E6DD]`}>View Details</button>
      </div>
    </div>
  );
