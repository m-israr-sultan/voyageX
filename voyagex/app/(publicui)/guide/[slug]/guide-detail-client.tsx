...
const GuideProfilePage = () => {
  ...
  useEffect(() => {
    if (guide?.id) {
      trackEvent("GUIDE_PROFILE_OPENED", "guide", guide.id);
    }
  }, [guide?.id]);
  ...
  const handleBookNow = () => {
    if (!guide || !isAvailable) return;
    trackEvent("BOOKING_STARTED", "guide", guide.id);
    router.push("/booking/travel-detail/" + guide.slug + "?type=guide");
  };
  ...
  const handleMessageClick = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        trackEvent("MESSAGE_STARTED", "guide", guide.id);
              ...
              <div className="p-4 sm:p-6">
                      ...
                      <div key={review.id} className="border-b border-gray-100 pb-4">
                        <div className="flex items-start gap-3">
                          <div className="relative w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center flex-shrink-0">
                            {review.users?.avatar ? <img src={getImageUrl(review.users.avatar)} alt="" loading="lazy" decoding="async" className="w-full h-full object-cover rounded-full" /> : <span className="text-gray-500 font-semibold text-sm sm:text-base">{review.users?.firstName?.[0] || "U"}</span>}
                          </div>
                ...
                {activeTab === "gallery" && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                    {guide.destinationImages?.length > 0 ? guide.destinationImages.map((img: string, i: number) => (<div key={i} className="relative aspect-video rounded-lg overflow-hidden"><img src={getImageUrl(img)} alt="Gallery" loading="lazy" decoding="async" className="w-full h-full object-cover" /></div>)) : (<p className="text-gray-600 col-span-full text-center py-4 text-sm sm:text-base">No gallery images available</p>)}
                  </div>
                )}
              </div>
            </div>
