...
export default function EarningsPage() {
  ...
  // Calculate guide earnings (85% of totalPrice)
  const calculateGuideEarnings = (totalPrice: number) => {
    return Math.round(totalPrice * 0.85);
  };

  const completedBookings = bookings.filter((b) => b.status === "COMPLETED");
  const totalEarnings = completedBookings.reduce((sum, b) => sum + calculateGuideEarnings(b.totalPrice || 0), 0);
  
  const pendingBookings = bookings.filter((b) => b.status === "CONFIRMED" || b.status === "IN_PROGRESS" || b.status === "AWAITING_TRAVELER_CONFIRMATION");
  const pendingEarnings = pendingBookings.reduce((sum, b) => sum + calculateGuideEarnings(b.totalPrice || 0), 0);
  
  const awaitingConfirmation = bookings.filter((b) => b.status === "AWAITING_TRAVELER_CONFIRMATION");
  const awaitingEarnings = awaitingConfirmation.reduce((sum, b) => sum + calculateGuideEarnings(b.totalPrice || 0), 0);

  // Monthly data for completed bookings (guide earnings)
  const monthlyData = completedBookings.reduce((acc: Record<string, { earnings: number; count: number }>, b) => {
    ...
    const earnings = calculateGuideEarnings(b.totalPrice || 0);
    if (!acc[month]) {
    ...
    return (
      <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Track your income and performance</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {completedBookings.slice(0, 5).map((booking) => {
              const earnings = calculateGuideEarnings(booking.totalPrice || 0);
              return (
                <div key={booking.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-green-600">+ Rs {earnings.toLocaleString()}</p>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700 mt-1">
              You earn <strong>85%</strong> of the total booking amount. The remaining 15% is the VoyageX platform fee.
              Payments are held in escrow and released after traveler confirms tour completion.
            </p>
          </div>
        </div>
      </div>
