...
export default function GuideVerificationPage() {
  ...
  const getDocStatus = (docType: string) => {
    // Prefer the most recent non-superseded document
    const docs = myDocs.filter((d) => d.type === docType && d.status !== "SUPERSEDED");
    if (docs.length === 0) return "MISSING";
    return docs[0].status;
  };

  const getDocFile = (docType: string) => {
    return myDocs.filter((d) => d.type === docType && d.status !== "SUPERSEDED")[0];
  };

  const getRejectionReason = (docType: string): string | null => {
    const doc = getDocFile(docType);
    return doc?.adminNote || null;
  };
  ...
  const requiredDocs = [
    { type: "CNIC_FRONT", label: "CNIC Front Side", icon: "🪪", description: "Clear photo of your CNIC front side" },
    { type: "CNIC_BACK", label: "CNIC Back Side", icon: "🪪", description: "Clear photo of your CNIC back side" },
    { type: "RECOMMENDATION_LETTER", label: "Recommendation Letter", icon: "📄", description: "From a registered agency, hotel, or local government authority" },
  ];

  const optionalDocs = [
    { type: "NOC", label: "NOC / Permission Letter", icon: "📋", description: "From local authorities or tourism department" },
    { type: "CERTIFICATE", label: "Certificates", icon: "🎓", description: "First aid, mountaineering, tourism training certificates" },
    { type: "BUSINESS_LICENSE", label: "Business License", icon: "🏢", description: "If operating as a registered business" },
    { type: "OTHER", label: "Other Documents", icon: "📎", description: "Any other supporting documents" },
  ];
  ...
  if (error) {
    return (
      <div className="space-y-4 sm:space-y-6 px-2 sm:px-0 max-w-2xl mx-auto">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Verification</h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Submit documents to get verified</p>
        </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0 max-w-2xl mx-auto">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Verification</h1>
        <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Submit documents to get your verified badge</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        ...
        {isFullyVerified ? (
          <p className="text-sm text-green-600 font-medium flex items-center gap-1 mt-2">
            <FaCheckCircle className="w-3 h-3" /> Congratulations! Your account is verified.
          </p>
        ) : (
          <p className="text-xs text-gray-500 mt-2">
            Upload all {totalRequired} required documents to get your "Verified" badge
          </p>
        )}
      </div>
      ...
      <div className="bg-blue-50 rounded-xl border border-blue-200 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-blue-800">Verification Benefits</h4>
