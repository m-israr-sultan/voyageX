...
export default function TravelerBookingsPage() {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const uniqueBookings = Array.from(
          new Map(result.data.map((b: any) => [b.id, b])).values()
        );
  ...
  const handleConfirmCompletion = async (bookingId: string) => {
    ...
    setError(null);
    try {
      const response = await bookingsApi.confirmCompletion(bookingId);
      const result = response.data;
      if (result.success) {
  ...
  if (error && bookings.length === 0) {
    return (
      <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">My Bookings</h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Track your tour bookings</p>
        </div>
    ...
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
          Track your tour bookings and confirm completion
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
        {[
          ...
          { label: "Need Action", count: stats.awaiting, color: "bg-yellow-50" },
          ...
          <button
            ...
            onClick={() =>
              setFilter(item.label === "Total" ? "ALL" : 
                item.label === "Need Action" ? "AWAITING_TRAVELER_CONFIRMATION" :
            ...
            className={`${item.color} rounded-lg border p-2 sm:p-3 text-center hover:shadow-md transition-all ${
              ...
              filter === (item.label === "Need Action" ? "AWAITING_TRAVELER_CONFIRMATION" : item.label.toUpperCase().replace(" ", "_"))
                ? "ring-2 ring-[#008A1E]"
                : ""
            }`}
          >
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-xs sm:text-sm text-gray-500">
                    <p className="flex items-center gap-1">
                      ...
                      Guide: {booking.packages?.guides?.users?.firstName}{" "}
                      {booking.packages?.guides?.users?.lastName || "To be assigned"}
                    </p>
                    <p className="flex items-center gap-1">
                    ...
                    <p className="font-semibold text-gray-900">
                      Amount: Rs {booking.totalPrice?.toLocaleString()}
                    </p>
